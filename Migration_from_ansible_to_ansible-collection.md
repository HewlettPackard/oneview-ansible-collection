...
(b)	Installation of HPE OneView Ansible Collection SDK
(c)	Running the Playbooks
(d)	Examples provided in the HPE OneView Ansible Collection 
...
# Requirements:
a)	As part of using the NEW Ansible Collection in the DevOps appliance, Customer will need to first uninstall the existing OLD Ansible module if an installed version exists and is <= 2.9. 
...
# (1) Install HPE OneView collection from GitHub:
```
...
# (2) Installation HPE OneView Ansible collection from Ansible Galaxy:
...
# (3) Installation HPE OneView Ansible collection from Red Hat Automation Hub:
...
# (4) Install HPE OneView collection from Docker Image:
...
## (e) Convert custom HPE OneView module playbooks to Ansible Collection [2.10]:
## (f) Developing new playbook in the HPE OneView Ansible Collection module:
...
# (Step 1) Creating a collection skeleton for the Custom Playbook:
...
3. Parameter values used in the tasks are read from oneview_custom_collection_use_case/defaults/main.yml and oneview_custom_collection_use_case/vars/main.yml.
...
# (Step 3) Build the Collection: To build the collection, run ansible-galaxy collection build from inside the root directory of the collection.
...
```
# (Step 5) Run the playbook as shown below:
``` 
